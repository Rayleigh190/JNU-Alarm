...
from ..models import Device, CollegeSet
...
#     return
#   posts = general_crawling(base_url=base_url, bbs_url=url, department_model=Engineering)
  
#   if len(posts) > 0:
#     for post in reversed(posts):
#       Engineering.objects.create(num=post['num'], title=post['title'])
#       # 공과대학을 구독한 User에게 알림 발송
#       isTrue_college =CollegeSet.objects.filter(engineering=True)
